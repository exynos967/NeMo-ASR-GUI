...
*   **다양한 포맷 출력**: 다음과 같은 여러 자막 및 텍스트 형식 생성을 지원합니다:
    *   `SRT`: 가장 널리 사용되는 표준 영화 자막 형식.
    *   `VTT`: 웹 비디오(HTML5) 표준 자막 형식.
    *   `ASS`: 고급 스타일, 효과 및 위치 지정을 지원하는 자막 형식 (고해상도 환경에 맞춰 최적화됨).
    *   `LRC`: 음악 플레이어용 동기화 가사 형식.
    *   `TXT / JSON`: 일반 텍스트 또는 구조화된 데이터로, 후처리, 보관 및 프로그램 연동에 용이.
